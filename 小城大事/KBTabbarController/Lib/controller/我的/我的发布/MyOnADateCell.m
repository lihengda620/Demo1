...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        self.titleLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), FitSizeH(8), FitSizeW(260), FitSizeH(30)) Font:17 IsBold:YES Text:@"王祖蓝" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
        [self.contentView addSubview:self.titleLabel];
        
        self.detailLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), FitSizeH(36) + CGRectGetMaxY(self.titleLabel.frame), FitSizeH(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"我正在寻找一位也在沈阳的20-30岁左右的女孩子..." Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
        [self.contentView addSubview:self.detailLabel];
        
        self.timeLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.detailLabel.frame) + FitSizeH(5), FitSizeW(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"2018/11/12 发布" Color:RGBA(150, 150, 150, 1.0) Direction:NSTextAlignmentLeft];
        ...
        self.dingBtn = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(222), CGRectGetMaxY(lineView1.frame) + FitSizeH(9), FitSizeW(22), FitSizeH(22)) Title:@"顶" Font:14 IsBold:YES TitleColor:RGBA(230, 43, 53, 1.0) TitleSelectColor:RGBA(230, 43, 53, 1.0) ImageName:nil Target:self Action:@selector(clickOnADateTopBtn:)];
...
- (void)clickOnADateTopBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickOnADateTopBtn:cell:)]) {
        [self.delegate clickOnADateTopBtn:button cell:self];
    }
}

- (void)clickOnADatelEditBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickOnADatelEditBtn:cell:)]) {
        [self.delegate clickOnADatelEditBtn:button cell:self];
    }
}

- (void)clickOnADateDeleteBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickOnADateDeleteBtn:cell:)]) {
        [self.delegate clickOnADateDeleteBtn:button cell:self];
    }
}
