...
@implementation MyHousekeepingCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        self.titleLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), FitSizeH(8), FitSizeW(260), FitSizeH(30)) Font:17 IsBold:YES Text:@"招聘育儿嫂保洁阿姨保姆" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
        ...
        self.companyNamelabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), FitSizeH(36) + CGRectGetMaxY(self.titleLabel.frame), FitSizeH(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"北京家政公司" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
        [self.contentView addSubview:self.companyNamelabel];
        
        self.timeLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.companyNamelabel.frame) + FitSizeH(5), FitSizeW(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"2018/11/12 发布" Color:RGBA(150, 150, 150, 1.0) Direction:NSTextAlignmentLeft];
        ...
        self.dingBtn = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(222), CGRectGetMaxY(lineView1.frame) + FitSizeH(9), FitSizeW(22), FitSizeH(22)) Title:@"顶" Font:14 IsBold:YES TitleColor:RGBA(230, 43, 53, 1.0) TitleSelectColor:RGBA(230, 43, 53, 1.0) ImageName:nil Target:self Action:@selector(clickHousekeepingTopBtn:)];
...
- (void)clickHousekeepingTopBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickHousekeepingTopBtn:cell:)]) {
        [self.delegate clickHousekeepingTopBtn:button cell:self];
    }
}

- (void)clickHousekeepingEditBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickHousekeepingEditBtn:cell:)]) {
        [self.delegate clickHousekeepingEditBtn:button cell:self];
    }
}

- (void)clickHousekeepingDeleteBtn:(UIButton *)button
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickHousekeepingDeleteBtn:cell:)]) {
        [self.delegate clickHousekeepingDeleteBtn:button cell:self];
    }
}
...
- (void)layoutSubviews
{
    ...
    NSString * timeStr = [AppTools getYMDHMSTimeString:[NSString stringWithFormat:@"%@",[self.dic objectForKey:@"createtime"]]];
