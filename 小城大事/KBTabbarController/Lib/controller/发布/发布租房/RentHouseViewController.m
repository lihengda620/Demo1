...
@interface RentHouseViewController ()<UIScrollViewDelegate,UITextViewDelegate,HXPhotoViewDelegate,UIGestureRecognizerDelegate,UITextFieldDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
//房屋配套
@property (nonatomic, strong) UITextField * peitaoTF;

//房屋户型
@property (nonatomic, strong) UIButton * huxingBtn;
@property (nonatomic, strong) UITextField * huxingTF;
...
- (void)initView{
    ...
    self.titleLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), 0, FitSizeW(200), FitSizeH(38)) Font:17.0f IsBold:YES Text:@"新建一条租房信息..." Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
    ...
    NSArray * userTextArr = @[@"房源名称",@"租金",@"支付方式",@"房屋配套",@"户型",@"产权面积",@"房源地址",@"提交房源照片"];
    for (int i = 0; i < userTextArr.count; i ++) {
        ...
        if (i != userTextArr.count - 1) {
            ...
            userInfoTextLabel.frame = CGRectMake(FitSizeW(15), FitSizeH(50) * i, FitSizeW(200), FitSizeH(50));
        }
    }
    
    //房源名称
    self.nameTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), 0, FitSizeW(270), FitSizeH(50)) placeholder:@"请输入房源名称" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    ...
    //租金
    self.zujinTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), FitSizeH(50), FitSizeW(270), FitSizeH(50)) placeholder:@"请输入月租金金额" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    self.zujinTF.backgroundColor = [UIColor clearColor];
    self.zujinTF.delegate = self;
    self.zujinTF.returnKeyType = UIReturnKeyDone;
    self.zujinTF.keyboardType = UIKeyboardTypeDecimalPad;
    [self.infoView addSubview:self.zujinTF];
    
    //付款方式
    self.payType = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(90), FitSizeH(100), FitSizeW(270), FitSizeH(50)) Title:@"请选择付款方式" Font:14 IsBold:NO TitleColor:RGBA(150, 150, 150, 1.0) TitleSelectColor:RGBA(150, 150, 150, 1.0) ImageName:@"tanchu" Target:self Action:@selector(click_payTypeBtn)];
    [self.payType setTitleEdgeInsets:UIEdgeInsetsMake(0, -FitSizeW(200), 0, 0)];
    [self.payType setImageEdgeInsets:UIEdgeInsetsMake(0, FitSizeW(250), 0, 0)];
    [self.infoView addSubview:self.payType];
    
    //房屋配套
    self.peitaoTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), FitSizeH(150), FitSizeW(270), FitSizeH(50)) placeholder:@"请填写房屋配套" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    self.peitaoTF.backgroundColor = [UIColor clearColor];
    self.peitaoTF.delegate = self;
    self.peitaoTF.returnKeyType = UIReturnKeyDone;
    [self.infoView addSubview:self.peitaoTF];
    
    //户型
    self.huxingTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), FitSizeH(200), FitSizeW(270), FitSizeH(50)) placeholder:@"请填写户型信息" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    self.huxingTF.backgroundColor = [UIColor clearColor];
    self.huxingTF.delegate = self;
    self.huxingTF.returnKeyType = UIReturnKeyDone;
    [self.infoView addSubview:self.huxingTF];
    
    //请填写产权面积
    self.mianjiTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), FitSizeH(250), FitSizeW(270), FitSizeH(50)) placeholder:@"请填写产权面积" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    self.mianjiTF.backgroundColor = [UIColor clearColor];
    self.mianjiTF.delegate = self;
    self.mianjiTF.returnKeyType = UIReturnKeyDone;
    self.mianjiTF.keyboardType = UIKeyboardTypeDecimalPad;
    [self.infoView addSubview:self.mianjiTF];
    
    //请填写楼层
    self.addressTF = [FlanceTools textFieldCreateTextFieldWithFrame:CGRectMake(FitSizeW(90), FitSizeH(300), FitSizeW(270), FitSizeH(50)) placeholder:@"请填写房源地址" passWord:NO leftImageView:nil rightImageView:nil Font:14.0 backgRoundImageName:nil];
    ...
    self.tishiTextView.text = @"承租者与您沟通后可以看到您的联系方式，同时您的消息中心也会收到通知.";
...
- (void)click_keyboardControl
{
    ...
    [self.zujinTF resignFirstResponder];
    [self.peitaoTF resignFirstResponder];
    [self.huxingTF resignFirstResponder];
    [self.addressTF resignFirstResponder];
    [self.mianjiTF resignFirstResponder];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"type":@"lease"};
    //AFN管理者调用get请求方法
...
#pragma mark 选择支付方式
- (void)click_payTypeBtn{
    [self KeyboardDown];
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"请选择付款方式" message:@"" preferredStyle:UIAlertControllerStyleActionSheet];
    [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
        NSLog(@"点击取消");
        
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"月付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"点击月付");
        [self.payType setTitle:@"月付" forState:UIControlStateNormal];
        [self.payType setImage:nil forState:UIControlStateNormal];
        [self.payType setTitleEdgeInsets:UIEdgeInsetsMake(0, -FitSizeW(240), 0, 0)];
        payTypeStr = @"1";
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"季付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"点击季付");
        [self.payType setTitle:@"季付" forState:UIControlStateNormal];
        [self.payType setImage:nil forState:UIControlStateNormal];
        [self.payType setTitleEdgeInsets:UIEdgeInsetsMake(0, -FitSizeW(240), 0, 0)];
        payTypeStr = @"2";
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"半年付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"点击半年付");
        [self.payType setTitle:@"半年付" forState:UIControlStateNormal];
        [self.payType setImage:nil forState:UIControlStateNormal];
        [self.payType setTitleEdgeInsets:UIEdgeInsetsMake(0, -FitSizeW(230), 0, 0)];
        payTypeStr = @"3";
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"年付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"点击年付");
        [self.payType setTitle:@"年付" forState:UIControlStateNormal];
        [self.payType setImage:nil forState:UIControlStateNormal];
        [self.payType setTitleEdgeInsets:UIEdgeInsetsMake(0, -FitSizeW(240), 0, 0)];
        payTypeStr = @"4";
    }]];
...
- (void)upLoadCommunityimage:(NSData *)imageData
{
    ...
    [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([self.isEditStr isEqualToString:@"1"] ? uploadImagePathArr.count == downLoadPhotoArr.count : uploadImagePathArr.count == photoImageArr.count) {
            ...
            if ([self.isEditStr isEqualToString:@"1"]) {
                [self doLeaseEdit:self.nameTF.text rental:self.zujinTF.text payment_way:payTypeStr configure:self.peitaoTF.text layout:self.huxingTF.text space:self.mianjiTF.text province:@"" city:currentCityStr area:@"" address:self.addressTF.text image:uploadImageAllPath mobile:self.shejiaoTF.text];
            }else{
                [self doLeaseCreate:self.nameTF.text rental:self.zujinTF.text payment_way:payTypeStr configure:self.peitaoTF.text layout:self.huxingTF.text space:self.mianjiTF.text province:@"" city:currentCityStr area:@"" address:self.addressTF.text image:uploadImageAllPath mobile:self.shejiaoTF.text];
            }
            [uploadImagePathArr removeAllObjects];
        }else{
            if ([self.isEditStr isEqualToString:@"1"]) {
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"lease",@"type_id":type_id,@"channel":@"2"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
...
#pragma mark 发布租房
- (void)doLeaseCreate:(NSString *)name rental:(NSString *)rental payment_way:(NSString *)payment_way configure:(NSString *)configure layout:(NSString *)layout space:(NSString *)space province:(NSString *)province city:(NSString *)city area:(NSString *)area address:(NSString *)address  image:(NSString *)image mobile:(NSString *)mobile
{   //创建请求地址
    NSString *url=[NSString stringWithFormat:@"%@",LeaseCreate];
    //构造参数
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"name":name,@"rental":rental,@"payment_way":payment_way,@"configure":configure,@"layout":layout,@"space":space,@"province":province,@"city":city,@"area":area,@"address":address,@"image":image,@"mobile":mobile};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
...
#pragma mark 编辑租房
- (void)doLeaseEdit:(NSString *)name rental:(NSString *)rental payment_way:(NSString *)payment_way configure:(NSString *)configure layout:(NSString *)layout space:(NSString *)space province:(NSString *)province city:(NSString *)city area:(NSString *)area address:(NSString *)address  image:(NSString *)image mobile:(NSString *)mobile
{   //创建请求地址
    NSString *url=[NSString stringWithFormat:@"%@",LeaseEdit];
    //构造参数
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"name":name,@"rental":rental,@"payment_way":payment_way,@"configure":configure,@"layout":layout,@"space":space,@"province":province,@"city":city,@"area":area,@"address":address,@"image":image,@"mobile":mobile,@"id":self.idStr};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
...
#pragma mark 获取租房详情
- (void)doGetLeaseDetails
{   //创建请求地址
    NSString *url=[NSString stringWithFormat:@"%@",LeaseDetails];
    //构造参数
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            //租金
            self.zujinTF.text = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"rental"]];
            
            //付款方式
            payTypeStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"payment_way"]];
            ...
            //房屋配套
            NSString * configureStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"configure"]];
            self.peitaoTF.text = configureStr;
            
            //房屋户型layout
            NSString * layoutStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"layout"]];
            self.huxingTF.text = layoutStr;
            
            //面积
            NSString * spaceStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"space"]];
            self.mianjiTF.text = spaceStr;
            
            //地址
            NSString * addressStr = [NSString stringWithFormat:@"%@%@%@",[dataDic objectForKey:@"city"],[dataDic objectForKey:@"area"],[dataDic objectForKey:@"address"]];
...
- (void)click_submitBtn:(UIButton *)sender
{
    ...
    if (self.nameTF.text.length < 1) {
        [self showAlert:@"请输入房源名称"];
        return;
    }
    if (self.zujinTF.text.length < 1) {
        [self showAlert:@"请输入租金金额"];
        return;
    }

    if (self.peitaoTF.text.length < 1) {
        [self showAlert:@"请填写房屋配套"];
        return;
    }
    if (self.huxingTF.text.length < 1) {
        [self showAlert:@"请填写户型信息"];
        return;
    }
    if (self.mianjiTF.text.length < 1) {
        [self showAlert:@"请填写产权面积"];
        return;
    }
    if (self.addressTF.text.length < 1) {
        [self showAlert:@"请填写房源地址"];
        return;
    }
    if (payTypeStr.length < 1) {
