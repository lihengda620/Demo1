...
@interface RentHouseDetailViewController ()<UIScrollViewDelegate>
...
- (void)initView{
    ...
    //房屋配套
    UILabel * liangdianTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.TypeLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"房屋配套" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    //房源地址
    UILabel * gongSiTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.liangDianLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"房源地址" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    self.houseAddressLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(gongSiLineView.frame), FitSizeW(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:self.houseAddressLabel];
    
    //房源实况
    UILabel * shikuangTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.houseAddressLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"房源实况" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:shikuangTextLabel];
    ...
    self.submitBtn = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(15), (TAB_BAR_HEIGHT - FitSizeH(44)) / 2, FitSizeW(345), FitSizeH(44)) Title:@"联系房东" Font:15 IsBold:NO TitleColor:[UIColor whiteColor] TitleSelectColor:[UIColor whiteColor] ImageName:nil Target:self Action:@selector(click_lookBtn)];
...
{
    NSString * titleStr = @"租房信息";
    NSString * descriptionStr = @"小城大事，致力于打造最专业的同城信息平台！";
    UIImage * setThumbImage = [UIImage imageNamed:@"tupian_zufang"];
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/Index/index/share/#/?id=%@&type=lease",RBDom,self.idStr];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"lease",@"type_id":type_id,@"channel":@"1"};
    //AFN管理者调用get请求方法
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            [self.submitBtn setTitle:@"联系房东" forState:UIControlStateNormal];
            //查看价格
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            NSString * salaryStr = [NSString stringWithFormat:@"%@ 元/月",[dataDic objectForKey:@"rental"]];
            ...
            NSString * payType = @"";
            if ([[NSString stringWithFormat:@"%@",[dataDic objectForKey:@"payment_way"]] isEqualToString:@"1"]) {
                payType = @"月付";
            }else if ([[NSString stringWithFormat:@"%@",[dataDic objectForKey:@"payment_way"]] isEqualToString:@"2"]) {
                payType = @"季付";
            }else if ([[NSString stringWithFormat:@"%@",[dataDic objectForKey:@"payment_way"]] isEqualToString:@"3"]) {
                payType = @"半年付";
            }else if ([[NSString stringWithFormat:@"%@",[dataDic objectForKey:@"payment_way"]] isEqualToString:@"4"]) {
                payType = @"年付";
            }
            
            NSString * typeStr = [NSString stringWithFormat:@"%@平米  |  %@  |  %@",[dataDic objectForKey:@"space"],[dataDic objectForKey:@"layout"],payType];
            ...
            NSString * addressStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"address"]];
            self.houseAddressLabel.text = addressStr;
            
            
            CGFloat h = FitSizeH(10) + CGRectGetMaxY(self.houseAddressLabel.frame) + FitSizeH(55);
            for (int i = 0; i < self.detailImageArr.count; i ++) {
                UIImageView * houseImageView = [FlanceTools imageViewCreateViewWithFrame:CGRectMake(FitSizeW(15), FitSizeH(10) + CGRectGetMaxY(self.houseAddressLabel.frame) + FitSizeH(55) + FitSizeH(210) * i, FitSizeW(345), FitSizeH(200)) ImageName:@""];
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"type_id":self.idStr,@"type":@"lease"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
