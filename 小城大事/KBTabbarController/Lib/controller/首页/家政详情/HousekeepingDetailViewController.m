//
//  HousekeepingDetailViewController.m
//  KBTabbarController
//
...
@interface HousekeepingDetailViewController ()<UIScrollViewDelegate>
...
@implementation HousekeepingDetailViewController{
...
- (void)initView{
    ...
    self.companyAddressLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.companyNameLabel.frame), FitSizeW(300), FitSizeH(20)) Font:14 IsBold:NO Text:@"" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
...
{
    NSString * titleStr = @"家政信息";
    NSString * descriptionStr = @"小城大事，致力于打造最专业的同城信息平台！";
    UIImage * setThumbImage = [UIImage imageNamed:@"tupian_jiazheng"];
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/Index/index/share/#/?id=%@&type=housekeep",RBDom,self.idStr];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"housekeep",@"type_id":type_id,@"channel":@"1"};
    //AFN管理者调用get请求方法
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"type_id":self.idStr,@"type":@"housekeep"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
