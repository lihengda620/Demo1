...
@interface BaySellDetailViewController ()<UIScrollViewDelegate>
...
@property (nonatomic, strong) UIView * gongSiLineView;
...
@implementation BaySellDetailViewController{
...
- (void)initView{
    ...
    self.mainScrollView.contentSize = CGSizeMake(0, FitSizeH(1400));
    ...
    //type
    self.TypeLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.dateLabel.frame), FitSizeW(340), FitSizeH(FitSizeH(30))) Font:14 IsBold:NO Text:@"同城面交" Color:RGBA(15, 15, 15, 1) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:self.TypeLabel];
    
    //房屋配套
    UILabel * liangdianTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.TypeLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"商品特征" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    //商品信息
    UILabel * miaoshuTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.liangDianLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"商品信息" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    //商品图片
    UILabel * gongSiTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.miaoShuLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"商品图片" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:gongSiTextLabel];
    
    self.gongSiLineView = [FlanceTools viewCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(gongSiTextLabel.frame), FitSizeW(345), FitSizeH(0.5)) BgColor:RGBA(220, 220, 220, 1.0) BgImage:nil];
    [self.mainScrollView addSubview:self.gongSiLineView];
    ...
    self.submitBtn = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(15), (TAB_BAR_HEIGHT - FitSizeH(44)) / 2, FitSizeW(345), FitSizeH(44)) Title:@"联系卖家" Font:15 IsBold:NO TitleColor:[UIColor whiteColor] TitleSelectColor:[UIColor whiteColor] ImageName:nil Target:self Action:@selector(click_lookBtn)];
...
{
    NSString * titleStr = @"买卖信息";
    NSString * descriptionStr = @"小城大事，致力于打造最专业的同城信息平台！";
    UIImage * setThumbImage = [UIImage imageNamed:@"tupian_maimai"];
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/Index/index/share/#/?id=%@&type=goods",RBDom,self.idStr];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"goods",@"type_id":type_id,@"channel":@"1"};
    //AFN管理者调用get请求方法
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            [self.submitBtn setTitle:@"联系卖家" forState:UIControlStateNormal];
            //查看价格
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            NSString * salaryStr = [NSString stringWithFormat:@"%@元",[dataDic objectForKey:@"price"]];
            ...
            NSString * configureStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"feature"]];
            self.liangDianLabel.text = configureStr;
            
            NSString * detailsStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"details"]];
            self.miaoShuLabel.text = detailsStr;
        
            
            CGFloat h = FitSizeH(10) + CGRectGetMaxY(self.gongSiLineView.frame);
            for (int i = 0; i < self.detailImageArr.count; i ++) {
                UIImageView * houseImageView = [FlanceTools imageViewCreateViewWithFrame:CGRectMake(FitSizeW(15), FitSizeH(10) + CGRectGetMaxY(self.gongSiLineView.frame) + FitSizeH(210) * i, FitSizeW(345), FitSizeH(200)) ImageName:@""];
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"type_id":self.idStr,@"type":@"goods"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
