...
@interface ElseDetailViewController ()<UIScrollViewDelegate>
...
@implementation ElseDetailViewController{
...
- (void)initView{
    ...
    self.titleLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), 0, FitSizeW(300), FitSizeH(38)) Font:17.0f IsBold:YES Text:@"" Color:RGBA(15, 15, 15, 1.0) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview: self.titleLabel];
    
    //发布内容
    UILabel * fabuneirongTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.titleLabel.frame) + FitSizeH(5), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"发布内容" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:fabuneirongTextLabel];
    
    UIView * fabuneirongLineView = [FlanceTools viewCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(fabuneirongTextLabel.frame), FitSizeW(345), FitSizeH(0.5)) BgColor:RGBA(220, 220, 220, 1.0) BgImage:nil];
    [self.mainScrollView addSubview:fabuneirongLineView];
    
    self.detailTV = [[UITextView alloc]initWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(fabuneirongLineView.frame) + FitSizeH(5), FitSizeW(345), FitSizeH(300))];
    self.detailTV.text = @"";
    self.detailTV.userInteractionEnabled = NO;
    self.detailTV.backgroundColor = [UIColor whiteColor];
    [self.detailTV setFont:[UIFont systemFontOfSize:14]];
    [self.detailTV setTextColor:RGBA(15, 15, 15, 1)];
    [self.mainScrollView addSubview:self.detailTV];
    ...
    self.submitBtn = [FlanceTools buttonCreateWithFrame:CGRectMake(FitSizeW(15), (TAB_BAR_HEIGHT - FitSizeH(44)) / 2, FitSizeW(345), FitSizeH(44)) Title:@"联系对方" Font:15 IsBold:NO TitleColor:[UIColor whiteColor] TitleSelectColor:[UIColor whiteColor] ImageName:nil Target:self Action:@selector(click_lookBtn)];
...
{
    NSString * titleStr = @"其他信息";
    NSString * descriptionStr = @"小城大事，致力于打造最专业的同城信息平台！";
    UIImage * setThumbImage = [UIImage imageNamed:@"tupian_qita"];
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/Index/index/share/#/?id=%@&type=else",RBDom,self.idStr];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"else",@"type_id":type_id,@"channel":@"1"};
    //AFN管理者调用get请求方法
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            [self.submitBtn setTitle:@"联系对方" forState:UIControlStateNormal];
            //查看价格
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"type_id":self.idStr,@"type":@"else"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
