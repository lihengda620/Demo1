...
@interface FriendDetailViewController ()<UIScrollViewDelegate>
...
//    self.mainScrollView.contentSize = CGSizeMake(0, FitSizeH(900));
    ...
    //发布日期
    self.dateLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.TypeLabel1.frame), FitSizeW(340), FitSizeH(FitSizeH(30))) Font:13 IsBold:NO Text:@"" Color:RGBA(150, 150, 150, 1) Direction:NSTextAlignmentLeft];
    [self.mainScrollView addSubview:self.dateLabel];
    
    
    
    //活动名称
    UILabel * liangdianTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.dateLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"活动名称" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    //活动内容
    UILabel * gongSiTextLabel = [FlanceTools labelCreateWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(self.liangDianLabel.frame), FitSizeW(200), FitSizeH(55)) Font:15 IsBold:YES Text:@"活动内容" Color:RGBA(80, 80, 80, 1.0) Direction:NSTextAlignmentLeft];
    ...
    self.zeyouBiaoZhunTV = [[UITextView alloc]initWithFrame:CGRectMake(FitSizeW(15), CGRectGetMaxY(gongSiLineView.frame) + FitSizeH(5), FitSizeW(345), FitSizeH(100))];
...
{
    NSString * titleStr = @"相约信息";
    NSString * descriptionStr = @"小城大事，致力于打造最专业的同城信息平台！";
    UIImage * setThumbImage = [UIImage imageNamed:@"tupian_xiangyue"];
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/Index/index/share/#/?id=%@&type=appointment",RBDom,self.idStr];
...
{   //创建请求地址
    ...
    NSDictionary *parameters = @{@"token":tokenStr,@"total_fee":total_fee,@"type":@"appointment",@"type_id":type_id,@"channel":@"1"};
    //AFN管理者调用get请求方法
...
{   //创建请求地址
    ...
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (code == 200) {
            ...
            NSString * typeStr = [NSString stringWithFormat:@"%@  |  %@岁  |  %@",[dataDic objectForKey:@"city"],[dataDic objectForKey:@"age"],sexStr];
            ...
            self.zeyouBiaoZhunTV.text = standardStr;

            NSString * play_profileStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"play_profile"]];
            self.jiaoyoujianjieTV.text = play_profileStr;
            
            NSString * play_contentStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"play_content"]];
            self.jiaoyouneirongTV.text = play_contentStr;
...
{   //创建请求地址
    ...
    NSDictionary * userinfoDic = [userDefaultes valueForKey:@"userinfoDic"];
    NSString * tokenStr = [NSString stringWithFormat:@"%@",[userinfoDic objectForKey:@"token"]];
    NSDictionary *parameters = @{@"token":tokenStr,@"type_id":self.idStr,@"type":@"appointment"};
    //AFN管理者调用get请求方法
    [[self sharedManager] POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        //返回请求返回进度
        NSLog(@"downloadProgress-->%@",uploadProgress);
