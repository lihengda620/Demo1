...
#import "FlanceTools.h"
#import <AVFoundation/AVFoundation.h>
#import <CommonCrypto/CommonDigest.h>

@implementation FlanceTools
...
+(UILabel *)labelCreateWithFrame:(CGRect)frame Font:(CGFloat)font IsBold:(BOOL)isBold  Text:(NSString *)text Color:(UIColor *)color Direction:(NSTextAlignment)direction
{
    ...
    if (isBold)
    {
       // label.font = [UIFont fontWithName:kFontStr size:font];  //设置label字体
        label.font = [UIFont boldSystemFontOfSize:font];
    }
    else
    {
        label.font = [UIFont systemFontOfSize:font];  //设置label字体
    }
...
+(UIView *)viewCreateWithFrame:(CGRect)frame BgColor:(UIColor *)bgColor BgImage:(UIImage *)bgImage
{
    ...
    if (bgColor)
    {
        view.backgroundColor = bgColor; //设置背景颜色
        
    }
    else
    {
        view.backgroundColor = [UIColor colorWithPatternImage:bgImage]; //设置背景图片
    }
...
{
    //阴影效果
    View.layer.shadowColor = RGBA(51, 51, 51, 1.0).CGColor;
...
{
    //阴影效果
    btn.layer.shadowColor = RGBA(51, 51, 51, 1.0).CGColor;
...
+(UIButton *)buttonCreateWithFrame:(CGRect)frame Title:(NSString *)title Font:(CGFloat)font IsBold:(BOOL)isBold TitleColor:(UIColor *)titleColor TitleSelectColor:(UIColor *)titleSelectColor ImageName:(NSString *)imageName Target:(id)target Action:(SEL)action
{
    ...
    if (isBold)
    {
        button.titleLabel.font = [UIFont fontWithName:kFontStr size:font];  //设置button字体
    }
    else
    {
        button.titleLabel.font = [UIFont systemFontOfSize:font];  //设置button字体
    }
    ...
    button.adjustsImageWhenDisabled = NO;   //按钮按下的时候系统要不要渲染
    ...
    if (imageName)
    {
        //        [button setBackgroundImage:[UIImage resizedImageWithName:imageName] forState:UIControlStateNormal];
    ...
    //    priceButton.imageEdgeInsets = UIEdgeInsetsMake(0, -5, 0, 5);
    //    priceButton.contentEdgeInsets = UIEdgeInsetsMake(0, 5, 0, -5);
...
//    [text setValue:(kMyUserTextLabelColor) forKeyPath:@"_placeholderLabel.textColor"];
    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:placeholder attributes:
    @{NSForegroundColorAttributeName:kMyUserTextLabelColor,NSFontAttributeName:text.font}];
    text.attributedPlaceholder = attrString;
    if (imageName == nil) {
        
        text.backgroundColor = PJColor(255, 255, 255);
    } else {
...
+(UITextField*)createTextFieldWithFrame:(CGRect)frame placeholder:(NSString*)placeholder passWord:(BOOL)YESorNO leftImageView:(UIImageView*)imageView rightImageView:(UIImageView*)rightImageView Font:(float)font
{
    ...
    //字体颜色
    textField.textColor=kMyUserTextLabelColor;
    return textField ;
...
+(NSString *)DoubleToString:(double)DoubleD
{
    ...
    NSString * dayOrderIncomeStr = [NSString stringWithFormat:@"%f",DoubleD];
    NSDecimalNumber *dn = [NSDecimalNumber decimalNumberWithString:dayOrderIncomeStr];
...
{
    NSString *Regex = @"[A-Z0-9a-z]{6}$";
    //    NSString *Regex = @"^(13[0-9]|14[0-9]|17[0-9]|15[0-9]|18[0-9])\\d{8}$";
    NSPredicate *phoneNumberText = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", Regex];
    return [phoneNumberText evaluateWithObject:mobile];
...
+ (BOOL) validateTelephone:(NSString*)telephone{
    NSString *phoneRegex = @"(^(0\\d{2})(\\d{8})$)|(^(0\\d{3})(\\d{7})$)";
    NSPredicate *telephoneTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",phoneRegex];
...
{
    NSString *nicknameRegex = @"([\u4e00-\u9fa5]{1,10})(&middot;[\u4e00-\u9fa5]{1,10})*";
...
{
    [self voiceChange:0.9];
    //初始化语音播报
    AVSpeechSynthesizer * av = [[AVSpeechSynthesizer alloc]init];
    //设置播报的内容
    AVSpeechUtterance * utterance = [[AVSpeechUtterance alloc]initWithString:VoiceTEXT];
    //设置语言类别
